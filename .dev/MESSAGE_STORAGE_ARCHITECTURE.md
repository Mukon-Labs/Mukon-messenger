...
## Future: On-Chain Encrypted Message Backup (Premium Feature)

**Date:** April 2026
**Status:** Architecture concept — not yet implemented

### The Idea

Offer optional on-chain encrypted message backup as a premium/paid feature, similar to how Signal offers paid cloud backup. Users who enable it get sovereign, censorship-resistant message history that survives device loss, app wipes, and even our servers going down.

**Free tier (current):** Local-first. Messages on device only. Backend is temporary relay.
**Premium tier (future):** Local-first + on-chain encrypted backup. Pay in $MU token.

### Why This Works

1. **Signal model validated** — Signal charges $1.99/month for cloud backup. Users pay for peace of mind. We offer something stronger: on-chain = no one can delete your backup.
2. **Storage costs are viable with compression** — Text messages are small (~200 bytes). With Light Protocol ZK compression (90-100x reduction), 1000 messages ≈ 2KB compressed ≈ ~$0.002 on Solana. Basically free per-message.
3. **$MU token utility** — Pay for backup storage in $MU. Real utility beyond speculation. Native payment rail already exists (wallet-based identity).
4. **Architecture is additive** — Doesn't change current local-first model at all. Just adds an optional encrypted write to chain on send/receive.

### Architecture

```
Current (free):
  Send message → encrypt → save to AsyncStorage → send via socket

Premium (on-chain backup enabled):
  Send message → encrypt → save to AsyncStorage → send via socket
                         → encrypt blob → store compressed on-chain (Light Protocol)

Recovery:
  Connect wallet → fetch compressed message accounts from chain → decrypt → full history
```
...
|--------|------|------|------|
| Light Protocol compressed accounts | ~$0.002/1K msgs | On Solana L1, sovereign, already coded | Devnet infra issues (prod-ready code) |
| Shadow Drive / Arweave + hash on Solana | Cheaper per byte | Better for media/large payloads | Extra dependency, less "on-chain" |
| IQ Labs compression | TBD | Alternative compression approach | Need to evaluate |

### Key Decisions Still Needed

- **Granularity:** Back up every message individually, or batch (e.g., daily chunks)?
- **Media:** Text-only first. Media backup is a separate problem (too expensive on-chain, would need Shadow Drive/Arweave for blobs + on-chain hash anchoring).
- **Token economics:** How much $MU per month/per message? Flat fee or usage-based?
- **Encryption:** Use same NaCl box/secretbox as current messages, or a separate backup key?
- **Retention:** Permanent (Arweave-style) or time-limited (cheaper)?

### Implementation Phases

1. **Phase 1:** `store_compressed_message` instruction (Light Protocol). Single message backup. Feature flag + settings toggle.
2. **Phase 2:** Batch backup (daily compressed chunks). More cost-efficient.
3. **Phase 3:** $MU token gate. Payment flow in settings.
4. **Phase 4:** Media backup via hybrid (blob on Shadow Drive/Arweave, hash on Solana).

### Competitive Advantage

- WhatsApp: Google Drive/iCloud backup (centralized, unencrypted by default)
- Signal: Paid cloud backup ($1.99/month, their servers)
- Mukon: On-chain encrypted backup (sovereign, censorship-resistant, pay in $MU)

**You own your messages. Not us, not Google, not Apple. The blockchain.**
